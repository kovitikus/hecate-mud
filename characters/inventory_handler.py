...
class InventoryHandler():
    ...
    def force_item_into_hand(self, item, hand=None):
        """
        This method is used to determine how to force an object into the character's main hand.

        Arguments:
            item (object, string): This is the item that is to go into the main hand. If a string is
                provided, this method will attempt to search the character's inventory and the
                character's location for a matching object.
        
        Keyword Arguments:
            hand (string): The hand that the item is to be assigned to. If this is None, the hand
                will default to main_hand.

        Returns:
            (None): If the item arg is a string and the search for a matching object has failed,
    ...
    def _save_hands(self):
        self.owner.db.hands = self.hands

    def inhand(self):
        owner = self.owner

        main_wield, off_wield, both_wield  = owner.db.wielding.values()
        main_hand, off_hand = self.hands.values()
        main_desc, off_desc = self.hands_desc.values()
